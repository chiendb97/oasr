...
"""``oasr.softmax`` and ``oasr.masked_softmax`` (``oasr/functionals/softmax.py``).

Row length is the only axis that changes the kernel's path -- it picks a
vector width and decides whether the row still fits the shared-memory cache --
so that is what the grids sweep.  ``test_vector_ladder_covers_odd_row_lengths``
is the deliberate exception to trimming: each of its eleven widths lands on a
different rung of the 8/4/2/1 ladder.
"""
...
class TestSoftmax:
    ...
    #: A row inside the shared-memory cache and one past it. Batch and
    #: sequence are grid parallelism over an independent per-row reduction.
    @pytest.mark.parametrize("channels", [256, 4096])
    @pytest.mark.parametrize("dtype", [torch.float32, torch.float16, torch.bfloat16])
    def test_softmax_correctness(self, channels, dtype):
        """Parity with ``F.softmax`` -- which already implies rows sum to one."""
    ...
    def test_softmax_destination_passing(self):
        x = torch.randn(2, 64, 128, device="cuda", dtype=torch.float16)
        assert_dest_passing(
            oasr.softmax,
            x,
            out=torch.empty_like(x),
            expected=F.softmax(x.float(), dim=-1).to(torch.float16),
        )
