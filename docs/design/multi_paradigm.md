...
| | |
|---|---|
| **Status** | Accepted — in execution (Phases 0–5 implemented on `feat/multi-paradigm-phase0`, 2026-07-17. Phase 1 note: transducer beam search + real-checkpoint WER parity deferred — no icefall transducer checkpoint on the dev box. Phase 2 note: U2++ rescoring validated against the WeNet reference loop on the real checkpoint; Whisper greedy validated on LJSpeech ground truth (`transformers` parity test importorskip-gated); AED beam search and paged decoder-KV integration into the `aed` strategy deferred — `DecoderKVCacheManager` (K6) ships tested but the strategy uses dense per-request KV. Phase 3 note: Paraformer validated tensor-by-tensor against FunASR 1.3.14 on `paraformer-zh` (encoder ≤1.6e-5, CIF fires bit-exact, token ids + transcript exact); timestamps are coarse CIF-fire boundaries (FunASR's `ts_prediction_lfr6_standard` smoothing not ported); FunASR variants (BiCif/SeACo/contextual hotwords), streaming chunked-CIF, and an AISHELL CER sweep deferred. Phase 4 note: speech-LLM (Qwen2-Audio) validated token-exact vs `transformers` greedy in fp32 on a tiny random fixture (single + left-padded batched) and content-identical on the real `Qwen2-Audio-7B-Instruct` in bf16 (same prefill argmax/top-5; greedy near-ties flip filler tokens only — LJ transcripts match ground truth); the `llm` strategy ships token-streaming partials over the incremental protocol. Phase 5 note: per-request `DecodingOptions` (n-best / `max_new_tokens` / temperature-top_k-top_p sampling / LLM prompt override) landed engine-side and through both serving surfaces — the proto's `max_alternatives` is now honored (engine-detokenized n-best), `confidence` = softmax-normalized n-best posterior, `result_end_time` ← CIF timestamps, plus `RecognitionConfig` extension fields 101–105 for the AR knobs; `oasr-server` gained `--decode-method` / `--llm-prompt` / `--max-new-tokens` / `--decode-steps-per-tick` / `--max-decode-slots`; the stateful streaming backend batches same-window streams via encoder-declared `stack/unstack_streaming_states` (3.5–24× at pool 4–32, argmax-identical); `Qwen2Lm` gained capacity-preallocated in-place KV (17% faster 7B step at B=8, token-identical) and the sustained-backlog starvation test asserts the tick-budget contract end-to-end. Deferred with measured rationale: paged decoder-KV via the CuteDSL FMHA (blocked on the masked-tile NaN fix — left-padded LLM prompts are exactly the failing key-padded shape) and decoder-step CUDA graphs (step ≈ 2× the weight-read floor → up to ~30% headroom; the static-KV prerequisite has landed, capture matrix (B × capacity buckets, EOS-shrinking B) is the remaining work); AED/transducer beam search unchanged) |
| **Date** | 2026-07-13 |
