...
|--------|---------|--------|
| `max_batch_size` | 32 | Encoder forward batch size — caps the running streaming pool **and** is the GPU forward width of the offline batch. Offline admission per step is capped at `max_batch_size` (one length-bucketed batch per `step()`). Remains the hard cap on resources when `preferred_batch_size` is set. |
...
2. **`schedule()` is O(N_offline)** per call where N_offline is the
   waiting queue length, bounded by `max_batch_size`. It does one
