...
static gemm::GemmStatus dispatchConv2dWithSmVersion(
    const ElementA* input_ptr, const ElementB* filter_ptr, const ElementCD* bias_ptr,
    ElementCD* output_ptr, int N, int H, int W, int IC, int K, int R, int S, int pad_h, int pad_w,
    int stride_h, int stride_w, int dilation_h, int dilation_w, cudaStream_t stream) {
    if constexpr (SM_VERSION == 75) {
        using Config = conv::CutlassConv2dConfig<16, 128, 64, 16, 32, 64, 3, 75>;
        return conv::CutlassConv2dFpropKernel<Config, ElementA, ElementB, ElementCD,
                                              activation_type>::run(input_ptr, filter_ptr, bias_ptr,
                                                                    output_ptr, N, H, W, IC, K, R,
                                                                    S, pad_h, pad_w, stride_h,
                                                                    stride_w, dilation_h,
...
template <int SM_VERSION, typename T>
static gemm::GemmStatus dispatchConv2dActivationWithSmVersion(
    const T* input, const T* filter, const T* bias, T* output, int N, int H, int W, int IC, int K,
    int R, int S, int pad_h, int pad_w, int stride_h, int stride_w, int dilation_h, int dilation_w,
    ActivationType activation, cudaStream_t stream) {
#define OASR_DISPATCH_CONV2D_ACTIVATION(ACTIVATION)                                          \
    dispatchConv2dWithSmVersion<SM_VERSION, T, T, T, ACTIVATION>(                            \
        input, filter, bias, output, N, H, W, IC, K, R, S, pad_h, pad_w, stride_h, stride_w, \
        dilation_h, dilation_w, stream)
    switch (activation) {
        case ActivationType::RELU:
            return OASR_DISPATCH_CONV2D_ACTIVATION(ActivationType::RELU);
        case ActivationType::GELU:
            return OASR_DISPATCH_CONV2D_ACTIVATION(ActivationType::GELU);
        case ActivationType::SWISH:
            return OASR_DISPATCH_CONV2D_ACTIVATION(ActivationType::SWISH);
        case ActivationType::IDENTITY:
            return OASR_DISPATCH_CONV2D_ACTIVATION(ActivationType::IDENTITY);
    }
#undef OASR_DISPATCH_CONV2D_ACTIVATION
    return gemm::GemmStatus::NOT_SUPPORTED;
}
...
cudaError_t Conv2D(const T* input, const T* filter, const T* bias, T* output, int N, int H, int W,
...
#ifdef OASR_TARGET_SM
    {
        ...
        status = detail::dispatchConv2dWithSmVersion<SM_VERSION, T, T, T,
                                                     oasr::ActivationType::IDENTITY>(
            input, filter, bias, output, N, H, W, IC, K, R, S, pad_h, pad_w, stride_h, stride_w,
            dilation_h, dilation_w, stream);
    }
#else
...
cudaError_t Conv2DActivation(const T* input, const T* filter, const T* bias, T* output,
    ...
    {
        constexpr int SM_VERSION = OASR_TARGET_SM;
        status = detail::dispatchConv2dActivationWithSmVersion<SM_VERSION, T>(
            input, filter, bias, output, N, H, W, IC, K, R, S, pad_h, pad_w, stride_h, stride_w,
            dilation_h, dilation_w, activation, stream);
    }
#else
    int sm = oasr::getDeviceSmVersion();
    OASR_DISPATCH_SM(sm, SM_VERSION, {
