...
struct FastDivmod {
  ...
  __host__ __device__ void operator()(int& quotient, int& remainder,
                                       int dividend) const {
    if (divisor == 0) {
      quotient = 0;
      remainder = 0;
    ...
    unsigned int t = (unsigned int)(
        (uint64_t)multiplier * (unsigned int)dividend >> 32);
...
__global__ void init_select_kernel(const float* __restrict__ log_prob,
                                   int batch_stride, int seq_stride,
                                   int vocab_stride,
                                   const int* __restrict__ seq_lengths,
                                   int batch, int max_seq_len, int blank_id,
                                   float log_threshold,
  ...
  for (int t_offset = 0; t_offset < actual_len;
       t_offset += BLOCK_SIZE * ITEMS_PER_THREAD) {
#pragma unroll
    for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ++ITEM) {
      int t = t_offset + ITEMS_PER_THREAD * tx + ITEM;
      if (t < actual_len) {
        float lp =
            log_prob[bid * batch_stride + t * seq_stride + blank_id * vocab_stride];
    ...
    BlockScanT{temp_storage}.ExclusiveSum(selected, selected_scan,
                                          block_agg_this_iter);
    __syncthreads();
...
#pragma unroll
  ...
  if (tx == 0) select_seq_lens[bid] = block_agg;
}
...
__global__ void first_step_kernel(
    const float* __restrict__ log_prob, int batch_stride, int seq_stride,
    int vocab_stride, const int* __restrict__ select_seqs,
    const int* __restrict__ select_seq_lens, float2* __restrict__ pprev,
    int* __restrict__ clast, int* __restrict__ clen, int* __restrict__ clist,
  ...
  for (int c = threadIdx.x; c < vocab_size; c += BLOCK_SIZE) {
    ...
    float p =
        log_prob[bid * batch_stride + first_t * seq_stride + c * vocab_stride];
    if (p > local_best) {
      local_best = p;
      local_best_id = c;
    }
  ...
  if (threadIdx.x == 0) {
    ...
    for (int c = 0; c < vocab_size; ++c) {
      ...
      for (int k = 1; k < beam; ++k)
        if (topk_keys[k] < topk_keys[min_idx]) min_idx = k;
    ...
    for (int k = 0; k < beam; ++k) {
      ...
      if (token >= 0 && token != blank_id) {
        ...
        float2 xy =
            need_add_blank ? make_float2(key, NEG_INF) : make_float2(NEG_INF, key);
        pprev[base] = xy;
...
__global__ void prob_matrix_kernel(
    const float* __restrict__ log_prob, int batch_stride, int seq_stride,
    int vocab_stride, const int* __restrict__ select_seqs,
    const int* __restrict__ select_seq_lens, int step,
    float2* __restrict__ pprev, float* __restrict__ ptable,
    float* __restrict__ ptablen, const int* __restrict__ clast, int ldc,
    int beam, int ldbeam, int batch, int blank_id, int space_id,
    int max_seq_len) {
  ...
  bool need_add_blank = (select_seqs[bid * max_seq_len + step] >
                         select_seqs[bid * max_seq_len + step - 1] + 1);
  ...
  for (; tid < total; tid += stride) {
    ...
    if (beam_idx >= beam) continue;
    // Blank and space are handled by prob_space_blank_kernel.
    if (char_idx == blank_id) continue;
    if (space_id >= 0 && char_idx == space_id) continue;
    ...
    float cur_prob =
        log_prob[bid * batch_stride + t * seq_stride + char_idx * vocab_stride];
...
__global__ void prob_space_blank_kernel(
    const float* __restrict__ log_prob, int batch_stride, int seq_stride,
    int vocab_stride, const int* __restrict__ select_seqs,
    const int* __restrict__ select_seq_lens, int step,
    float2* __restrict__ pprev, float* __restrict__ ptable,
  ...
  int blank_slot = blank_id + (bid * beam + beam_idx) * ldc;
  ptable[blank_slot] =
      logprob_add(blank_prob, logsumexp(prev.x, prev.y));
  ...
  if (space_id >= 0 && space_id != blank_id) {
    float space_prob =
        log_prob[bid * batch_stride + t * seq_stride + space_id * vocab_stride];
    int space_slot = space_id + (bid * beam + beam_idx) * ldc;
    ptablen[space_slot] =
        logprob_add(space_prob, logsumexp(prev.x, prev.y));
    ptable[space_slot] = NEG_INF;
  }
}
...
__global__ void merge_kernel(
    ...
    const int* __restrict__ clen, int ldc, int beam, int ldbeam, int ldseq_len,
  ...
  if (longer_beam < beam &&
      (smem_clen[longer_beam] - 1) == smem_clen[shorter_beam]) {
    ...
    if (seq_compare(
      ...
      int tidin =
          clast[longer_beam + bid * ldbeam] + (shorter_beam + bid * beam) * ldc;
...
template <int BLOCK_SIZE, int ITEMS_PER_THREAD>
__global__
__launch_bounds__(BLOCK_SIZE) void topk_phase1_kernel(
    const int* __restrict__ select_seq_lens, int step,
    const float* __restrict__ ptable, const float* __restrict__ ptablen,
    int ldc, int beam, int batch, float* topk_key_buffer,
    int* topk_value_buffer) {
  const int bid = blockIdx.y;
  if (bid >= batch) return;
  if (step >= select_seq_lens[bid]) return;
  ...
  typedef cub::BlockRadixSort<float, BLOCK_SIZE, ITEMS_PER_THREAD, int>
      BlockSortT;
...
__global__
__launch_bounds__(BLOCK_SIZE) void topk_phase2_kernel(
    const int* __restrict__ select_seq_lens, int step,
    int items_per_batch,  // = bxs * beam
    int beam, int batch, float* __restrict__ topk_key_buffer,
    int* __restrict__ topk_value_buffer, int ldc, int ldbeam, int ldseq_len,
    float2* __restrict__ pprev, const float* __restrict__ ptable,
    const float* __restrict__ ptablen, int* __restrict__ clast,
    int* __restrict__ clen_src, int* __restrict__ clen_dst,
    int* __restrict__ clist_src, int* __restrict__ clist_dst,
    float* __restrict__ score, int blank_id,
    ...
    if (tid_in_sub == 0) {
      ...
      if (char_id == blank_id) {
        ...
        if (prevlen < ldseq_len) {
          clist_dst[bid * beam * ldseq_len + out_beam * ldseq_len + prevlen] =
              char_id;
        }
      }
...
__global__ void fixup_parity_kernel(
  ...
  if (batch_parity == final_parity) return;
...
inline size_t calculate_workspace_size(int batch, int beam, int vocab_size,
                                        int max_seq_len) {
...
inline void init_internal_data(InternalData* data, void* workspace, int batch,
  ...
  char* ptr = reinterpret_cast<char*>(
      (reinterpret_cast<uintptr_t>(workspace) + ALIGN_BYTES - 1) / ALIGN_BYTES *
      ALIGN_BYTES);
...
inline cudaError_t ctc_prefix_beam_search_step(
    InternalData* data, const float* log_prob, int batch_stride,
    int seq_stride, int vocab_stride, int step, bool is_last_step, int blank_id,
  ...
  if (step == 0) {
    ...
    int smem_size =
        (sizeof(float) + sizeof(int)) * 256;  // s_keys + s_vals for BLOCK_SIZE=256
    first_step_kernel<256><<<batch, 256, smem_size, stream>>>(
        ...
        data->clist[0], data->score, beam, ldbeam, ldseq_len, data->vocab_size,
        blank_id, batch, max_seq_len);
  } else {
    // --- 1. Compute probability matrix (non-blank chars) ---
    {
      ...
      prob_matrix_kernel<<<grid, threads, 0, stream>>>(
          log_prob, batch_stride, seq_stride, vocab_stride, data->select_seqs,
          data->select_seq_lens, step, data->pprev, data->ptable, data->ptablen,
          data->clast, ldc, beam, ldbeam, batch, blank_id, space_id,
          max_seq_len);
    }
    ...
    // --- 3. Merge duplicate prefixes ---
    {
      ...
      merge_kernel<<<merge_grid, ldbeam, 0, stream>>>(
          ...
          data->clist[src_parity], data->clen[src_parity], ldc, beam, ldbeam,
          ldseq_len, batch, blank_id);
    }
    ...
    // --- 4. Top-K selection: Phase 1 (multi-block per batch) ---
    {
      ...
      int bxs = min(MAX_BLOCKS_PER_BATCH,
                    max(1, (all_items + P1_BLOCK * P1_IPT - 1) /
                               (P1_BLOCK * P1_IPT)));
      bxs = min(bxs, MAX_BLOCKS / max(batch, 1));
      dim3 p1_grid(bxs, batch);
      topk_phase1_kernel<P1_BLOCK, P1_IPT><<<p1_grid, P1_BLOCK, 0, stream>>>(
          data->select_seq_lens, step, data->ptable, data->ptablen, ldc, beam,
          batch, data->topk_key_buffer, data->topk_value_buffer);
      ...
      topk_phase2_kernel<P2_BLOCK, P2_IPT><<<batch, P2_BLOCK, 0, stream>>>(
          ...
          data->clist[src_parity], data->clist[dst_parity], data->score,
...
inline cudaError_t ctc_beam_search_decode_batch(
    ...
    void* workspace, int batch, int beam, int vocab_size, int max_seq_len,
    int max_out_len, int blank_id, int space_id, float blank_threshold,
    cudaStream_t stream) {
  ...
  cudaMemsetAsync(data.ptable, 0xcc,
                  sizeof(float) * batch * beam * data.ldc, stream);
  cudaMemsetAsync(data.ptablen, 0xcc,
                  sizeof(float) * batch * beam * data.ldc, stream);
  ...
  init_select_kernel<SEL_BLOCK, SEL_IPT><<<batch, SEL_BLOCK, 0, stream>>>(
      log_prob, batch_stride, seq_stride, vocab_stride, seq_lengths, batch,
      data.max_seq_len, blank_id, log_threshold, data.select_seqs,
      data.select_seq_lens);
  ...
  for (int step = 0; step < max_select; ++step) {
    ...
    cudaError_t err = ctc_prefix_beam_search_step(
        &data, log_prob, batch_stride, seq_stride, vocab_stride, step, is_last,
        blank_id, space_id, stream);
  ...
  fixup_parity_kernel<<<batch, 32, 0, stream>>>(
      data.select_seq_lens, max_select, data.clen[0], data.clen[1],
      data.clist[0], data.clist[1], data.ldbeam, data.ldseq_len, beam, batch,
      final_parity);
  ...
  cudaMemcpy2DAsync(out_tokens, sizeof(int) * max_out_len,
                    ...
                    cudaMemcpyDeviceToDevice, stream);
  cudaMemcpy2DAsync(out_scores, sizeof(float) * beam, data.score,
                    sizeof(float) * data.ldbeam, sizeof(float) * beam, batch,
                    cudaMemcpyDeviceToDevice, stream);
...
static constexpr size_t STATE_HEADER_SIZE =
    align_size(sizeof(InternalData) + sizeof(int) * 4);

inline size_t calculate_state_buffer_size(int batch, int beam, int vocab_size,
                                           int max_seq_len) {
  return STATE_HEADER_SIZE +
         calculate_workspace_size(batch, beam, vocab_size, max_seq_len);
}
...
inline void init_streaming_state(void* state_buffer, int batch, int beam,
                                  int vocab_size, int max_seq_len, int blank_id,
                                  cudaStream_t stream) {
  ...
  void* workspace =
      reinterpret_cast<char*>(state_buffer) + STATE_HEADER_SIZE;
  init_internal_data(&state.data, workspace, batch, beam, vocab_size,
                     max_seq_len);
  ...
  cudaMemsetAsync(state.data.clist[1], 0xff,
                  sizeof(int) * batch * beam * state.data.ldseq_len, stream);
  cudaMemsetAsync(state.data.ptable, 0xcc,
                  sizeof(float) * batch * beam * state.data.ldc, stream);
  cudaMemsetAsync(state.data.ptablen, 0xcc,
                  sizeof(float) * batch * beam * state.data.ldc, stream);
  ...
  for (int b = 0; b < batch; ++b) h_sel_lens[b] = max_seq_len;
  cudaMemcpyAsync(state.data.select_seq_lens, h_sel_lens,
  ...
  cudaMemcpyAsync(state.data.select_seqs, h_sel_seqs,
                  sizeof(int) * batch * max_seq_len, cudaMemcpyHostToDevice,
                  stream);
  delete[] h_sel_seqs;
...
inline cudaError_t streaming_step(void* state_buffer,
                                   const float* log_prob_frame,
                                   int batch_stride, int vocab_stride, int step,
  ...
  return ctc_prefix_beam_search_step(
      &state.data, log_prob_frame, batch_stride, 0, vocab_stride, step,
      is_last, blank_id, space_id, stream);
}

inline cudaError_t read_streaming_results(void* state_buffer, int* out_tokens,
                                           int* out_lengths, float* out_scores,
                                           int max_out_len,
                                           cudaStream_t stream) {
  StreamingState state;
  cudaMemcpyAsync(&state, state_buffer, sizeof(StreamingState),
