...
- **`layers/`** — **The narrow waist every model implementation is built from** (H1). `conv.py`, `linear.py`, `norm.py`, `embedding.py`, `mlp.py`, `feature.py`, `softmax.py`, `topk.py`, `attention/`, `rotary_embedding/`, plus `ctc.py` (`CtcProjection` — fused `log_softmax(x @ Wᵀ + b)` via `oasr.gemm_log_softmax`, used by `CTCHead`). Models use these instead of `nn.Linear`/`nn.LayerNorm`/`nn.Embedding` so kernels, CUDA graphs and future quantization apply to **all six** architectures rather than one and a half. Members beyond the wrappers: `Attention` (`attention/core.py` — generic non-rel-pos MHA over *pre-projected, head-split* q/k/v; the projections stay on the model under their checkpoint's names, so the shared thing is the compute, not the key layout), `FeedForward`/`GatedMLP`, `Embedding`, `ColumnParallelLinear`/`RowParallelLinear` (single-GPU today — they record the TP shard axis at the definition site so M3 is a fill-in), `NeoxRotaryEmbedding`+`apply_rotary_pos_emb` (HF-style real cos/sin over **arbitrary per-row positions** — the left-padded-prompt case the complex `freqs_cis` API cannot express), and the eps constants `TORCH_EPS`/`ESPNET_EPS`/`QWEN2_RMS_EPS`. **Each layer owns a kernel path *and* a torch path** and chooses per call in `_backend.py`: GEMM needs CUDA + fp16/bf16 + both dims 8-aligned (CUTLASS alignment-8 iterators *raise* otherwise — Paraformer's 8404 vocab and icefall's 500 land on torch), norms need CUDA + contiguous (they address rows arithmetically, so a conv encoder's `transpose(1,2)` output cannot use them), and `Attention` asks `jit.attention.fmha_config_supported` before dispatching because `oasr.fmha` raises on a head_dim the CuteDSL kernel cannot compile (128 on sm_120). That dual path is what lets one model file serve fp16 on the GPU and run the fp32 CPU parity oracles. `OASR_LAYERS_BACKEND` overrides it. `Attention` takes masks as `kv_lens` (right padding — pushed into the kernel as a length vector, no materialized bias), `attn_bias` (`(B,H,T_q,T_k)` additive), `attn_mask` (anything SDPA takes) or `is_causal`; the last two are SDPA-only by construction, not by omission. **The fused kernel is used only when there is a mask to fuse**: measured fp16 on a 5090, `kv_lens` shapes are 1.7-1.9x *faster* fused, while unmasked ones are 1.25-1.87x *slower* than SDPA — the win is the fusion, so with nothing to fuse torch's flash path is simply better (this is why Whisper, whose attention is never masked, stays on SDPA end to end). The GEMM predicate carries the matching rule on the other axis: a **work floor** (`GEMM_MIN_MACS`, 2^24 MACs) below which reaching CUTLASS costs more in fixed Python dispatch (~20us: heuristic lookup, workspace, reshapes, the `@oasr_api` wrapper) than the kernel saves. The rule is a pure function of the call and is deliberately **not** relaxed under CUDA-graph capture even though the dispatch cost is free there: a capture-dependent branch makes the graph pick a different kernel than eager, and that one-ulp fp16 difference came out of the transducer decoder as *different tokens* (`test_streaming_graph_capture_is_token_identical_to_eager` catches it). Effect on an eager AR decode step: whisper-tiny 30 steps 75.0 -> 67.4 ms. Net end-to-end: conformer and zipformer within noise, whisper-tiny p50 +5%. The two real fixes — cheaper GEMM dispatch and CUDA-graph capture of the AR step — are both already tracked follow-ups. `LinearActivation` accepts `gelu_tanh` but **not** `gelu`: the CUDA epilogue is the tanh approximation and Whisper/Qwen2-Audio are trained on exact erf, so fusing it under that name would be a silent accuracy change.
...
| `OASR_LAYERS_BACKEND` | `auto` (default — `oasr.layers` uses a kernel where it can run, torch otherwise), `torch` (never call a kernel: the debugging fallback, so a numerical difference that survives it is not the kernels' fault), `oasr` (kernel or raise, for GEMM and norm — how a benchmark *proves* a model is on the kernel path instead of assuming it) |
