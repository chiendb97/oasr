...
- **`engine/`** — Inference engine for offline + streaming Conformer-CTC on a single GPU:
  ...
  - Internal modules: `scheduler.py`, `model_runner.py`, `input_processor.py`, `output_processor.py`, plus the `executor/` package — `base.py` (`Executor` ABC), `offline.py` (`OfflineExecutor`: length-bucketed micro-batches, plus opt-in **sequence packing** — when `enable_sequence_packing` is set each micro-batch is one gapless varlen-attention packed row), `streaming.py` (`StreamingExecutor`: chunk-by-chunk paged-KV). Service mode pinning: `EngineConfig.service_mode ∈ {"streaming","offline"}` selects exactly one executor per engine lifecycle; mismatched requests are rejected at admission.
