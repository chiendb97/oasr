    ...
    * ``num_threads = 128`` (4 warps).
    ...
    def __init__(
        ...
        *,
        ...
        # Per-row key *start* (left padding).  A compile-time flag so the mask
        # specializes: `window_left` cannot express it, being a constexpr, while
        # the pad amount differs per row.
        self._has_seqstart = has_seqstart
    ...
    #: Shallowest ring the *layouts* support, which is not the same as the
    #: shallowest the mainloop's index arithmetic would accept.  ``sV`` is a
    #: 3-mode smem tensor ``(N, D, stages)`` and the epilogue takes
    #: ``sVt[None, None, 0]``; at ``stages == 1`` cute simplifies the size-1
    #: stage mode away and that index fails IR verification ("coord and shape
    #: of view are weakly congruent").  Nothing exercised a 1-deep ring until
    #: the smem budget started choosing one, at which point ``can_implement``
    #: was cheerfully advertising a config that could not compile.  Narrowing
    #: the K tile is the cheaper concession, so the floor stays here.
    MIN_NUM_STAGES = 2
    ...
    #: Narrower N tiles to fall back on for a head_dim whose requested tile
    #: cannot fit any ring depth, descending.
    #:
    #: 16 is the floor, not 8: the m16n8k16 atom is tiled across 4 warps, so a
    #: 64x8 S tile leaves the QK accumulator's N mode not matching the
    #: fragment's and ``cute.gemm`` refuses it ("expects B's N == D's N").
    #: With a 64-row M tile that caps the reachable head_dim at 320.
    _N_BLOCK_FALLBACKS = (64, 32, 16)
    ...
    def select_tile(
        ...
        *,
        ...
        """``(n_block, num_stages)`` that fits this arch's smem, or ``(0, 0)``.

        The requested ``n_block_size`` is tried first and at the deepest ring
        that fits, so nothing already working changes.  Only when *no* ring
        depth fits does the N tile narrow — which is what closes very wide
        heads: at ``head_dim=512`` a 64x64 tile needs 192 KB against a 99 KB
        cap even with a single stage, while 64x16 needs 96 KB.  Narrowing N
        costs iterations (more, smaller K tiles) and is therefore a last
        ...
        than producing a zero-block walk.
        """
        for n_block in cls._N_BLOCK_FALLBACKS:
            if n_block > n_block_size:
                continue
            if paged and (block_size <= 0 or n_block % block_size != 0):
                continue
            stages = cls.select_num_stages(
                head_dim=head_dim,
    ...
    def select_num_stages(
        ...
        *,
        ...
        for stages in range(limit, cls.MIN_NUM_STAGES - 1, -1):
            need = cls.smem_bytes(
                head_dim=head_dim,
                m_block_size=m_block_size,
        ...
        **_kwargs,
        ...
        if paged:
            ...
            if varlen:
                return False
        if varlen:
            # Packed varlen path skips per-element head-dim predication on the
            # offset-shifted views; require head_dim on the MMA k-stride.
            if head_dim % 32 != 0:
                return False
        if causal and window_size_right == -1:
            window_size_right = 0  # causal implies right-window 0
        del has_seqstart  # a mask predicate; costs no shared memory
        ...
        # boolean.
        if num_stages is None:
            # Ask the same question the compile path asks: is there *any*
            # (n_block, ring depth) that fits?  Checking only the requested
            # n_block would refuse a wide head that merely needs a narrower
            # K tile.
            return (
                cls.select_tile(
                    head_dim=head_dim,
    ...
    def __call__(
        ...
        if cutlass.const_expr(self._q_in_regs):
            ...
            @cute.struct
            class SharedStorage:
                sQ: cute.struct.Align[cute.struct.MemRange[self._dtype, cosize_sQV], 1024]
    ...
    def kernel(
        ...
        # Left padding: keys below this index are pad.  Zero when the caller
        # passed no starts (the wrapper hands over a dummy in that case), so the
        # mask predicate compiles out entirely.
        seqstart_k = cutlass.Int32(0)
        ...
        if cutlass.const_expr(self._window_size_left >= 0):
            left_col = m_block * self._m_block_size - self._window_size_left
            n_block_min = cutlass.max(n_block_min, left_col // self._n_block_size)
        # Leave at least one block: a work-free CTA is safe here (acc_O is zeroed
        # and the softmax epilogue clamps a zero row_sum), but the mainloop below
        # runs its first block unconditionally, so clamping is what keeps the
        # cp.async prologue and its barriers on one code path.  A fully-masked
        # block costs one iteration and returns zeros via the empty-row clamp.
        ...
        tOrVt = thr_mma.make_fragment_B(thr_mma.partition_B(sVt[None, None, 0]))
        ...
        for stage in cutlass.range_constexpr(self._num_stages):
            ...
            if cutlass.const_expr(not self._q_in_regs) or stage > 0:
                if stage == 0 or n_block_init - stage >= n_block_min:
                    self._load_kv_tile(
                        mK,
                        sK,
                        tKsK,
        ...
        mask = AttentionMask(
            ...
            has_seqlen_k=True,
            has_seqlen_q=True,
            has_seqstart_k=self._has_seqstart,
        )
        seqlen_q = mQ.shape[2]
    ...
    # ------------------------------------------------------------------------
    # Variable-length (sequence-packed) host launcher + device kernel
    # ------------------------------------------------------------------------
    @cute.jit
    def _seg_view(
        self,
        ...
        """A ``(1, heads, seqlen, D)`` view of packed ``(total, heads, D)`` at ``offset``.

        Lets the dense prologue/mainloop/epilogue + helpers run unchanged on
        one segment: ``.shape[2] == seqlen`` so every length predicate (Q load,
        K residue, O store) masks at the segment boundary, and
        ``mView[0, head, None, None]`` yields the ``(seqlen, D)`` tile.  The
        explicit ``seqlen`` is required — ``domain_offset`` keeps the packed
        ``shape[0] == total``, which would let the epilogue store past the
        segment into the next one.
        """
        mShift = cute.domain_offset((offset, 0, 0), mPacked)
        lay = cute.make_layout(
            (1, mShift.shape[1], seqlen, mShift.shape[2]),
            stride=(0, mShift.stride[1], mShift.stride[0], mShift.stride[2]),
        )
        return cute.make_tensor(mShift.iterator, lay)

    @cute.jit
    def _seg_bias_view(
        self,
        mBiasFlat: cute.Tensor,
        bias_off: cutlass.Int32,
        seqlen_q: cutlass.Int32,
        seqlen_k: cutlass.Int32,
    ) -> cute.Tensor:
        """A ``(1, H, seqlen_q, seqlen_k)`` view of the packed block-diagonal bias.

        Segment ``s``'s block lives contiguously (row-major ``(H, T_q, T_k)``)
        at flat offset ``bias_offsets[s]`` in ``mBiasFlat``; this reinterprets
        it so ``_add_bias_tile`` indexes ``mBias[0, head, ...]`` unchanged.
        """
        mShift = cute.domain_offset((bias_off,), mBiasFlat)
        lay = cute.make_layout(
            (1, self._num_heads, seqlen_q, seqlen_k),
            stride=(0, seqlen_q * seqlen_k, seqlen_k, 1),
        )
        return cute.make_tensor(mShift.iterator, lay)
    ...
    def forward_varlen(
        ...
        mK: cute.Tensor,  # (total_k, H_kv, D)
        mV: cute.Tensor,  # (total_k, H_kv, D)
        mO: cute.Tensor,  # (total_q, H,    D)
        mBias: cute.Tensor,  # packed block-diagonal bias or 0-rank dummy
        mBiasOff: cute.Tensor,  # (S+1,) int32 bias block offsets or dummy
        mCuSeqlensQ: cute.Tensor,  # (S+1,) int32
        mCuSeqlensK: cute.Tensor,  # (S+1,) int32
        ...
        # Grid: (m_blocks over the longest segment, num_segments, H).  Tiles
        # whose m_block falls past their own segment early-exit in the kernel.
        num_m_blocks = cute.ceil_div(max_seqlen_q, self._m_block_size)
        num_segs = mCuSeqlensQ.shape[0] - 1
        grid_dim = (num_m_blocks, cute.size(num_segs), cute.size(mQ.shape[1]))
    ...
    def kernel_varlen(
        ...
        mK_packed: cute.Tensor,
        mV_packed: cute.Tensor,
        mO_packed: cute.Tensor,
        mBias: cute.Tensor,
        mBiasOff: cute.Tensor,
        ...
        tidx, _, _ = cute.arch.thread_idx()
        m_block, batch_seg, num_head = cute.arch.block_idx()

        pack_gqa = PackGQA(qhead_per_kvhead=self._gqa_ratio)
        kv_head = pack_gqa.kv_head_of(num_head)

        # Per-segment offsets/lengths from cu_seqlens.
        seqlen_q = mCuSeqlensQ[batch_seg + 1] - mCuSeqlensQ[batch_seg]
        offset_q = mCuSeqlensQ[batch_seg]
        seqlen_k = mCuSeqlensK[batch_seg + 1] - mCuSeqlensK[batch_seg]
        offset_k = mCuSeqlensK[batch_seg]

        # Early-exit tiles whose Q rows are all past this segment.
        if m_block * self._m_block_size < seqlen_q:
            # Per-segment 4D views so the dense body + helpers run unchanged.
            mQ = self._seg_view(mQ_packed, offset_q, seqlen_q)
            mK = self._seg_view(mK_packed, offset_k, seqlen_k)
            mV = self._seg_view(mV_packed, offset_k, seqlen_k)
            mO = self._seg_view(mO_packed, offset_q, seqlen_q)
            if cutlass.const_expr(self._has_bias):
                mBias_seg = self._seg_bias_view(
                    mBias,
                    mBiasOff[batch_seg],
                    seqlen_q,
                    seqlen_k,
                ...
                mBias_seg = mBias
            mBlockTable = None
            batch_size = cutlass.Int32(0)
            t_kv_logical = seqlen_k
            # Varlen exposes neither a key start nor a window, so its low blocks
            # are never fully masked and the loop's lower bound stays 0.
            seqstart_k = cutlass.Int32(0)
            n_block_min = cutlass.Int32(0)
            n_block_max = cute.ceil_div(seqlen_k, self._n_block_size)

            # Tier 1B, mirroring the dense mainloop (see there for the
            # rationale).  Inert today: ``fmha_varlen`` exposes no ``causal``
            # argument, so ``self._causal`` is always False on this path and the
            # branch never traces.  Kept so the two mainloops do not silently
            # diverge the day packed causal attention is wanted.
            if cutlass.const_expr(self._causal):
                n_block_causal = cute.ceil_div(
    ...
    def _advance_pipeline(self, pipeline_index):
        ...
        return pipeline_index + 1 if pipeline_index < self._num_stages - 1 else cutlass.Int32(0)
    ...
    def _compute_one_n_block(
        ...
        if cutlass.const_expr(self._num_stages == 1) or (
            n_block - self._num_stages + 1 >= n_block_min
        ):
            n_block_v_next = (
                n_block
        ...
        # ---- Mask + online softmax ----------------------------------------
        mcS = cute.make_identity_tensor((mQ.shape[0], mQ.shape[1], mQ.shape[2], t_kv_logical))
        cS = cute.local_tile(
        ...
        mask.apply(acc_S, tScS, seqlen_q=seqlen_q, seqlen_k=seqlen_k, seqstart_k=seqstart_k)
        ...
        if cutlass.const_expr(self._num_stages > 1):
            cute.arch.cp_async_wait_group(wait_count)
            self.cta_sync_barrier.arrive_and_wait()
            if n_block - self._num_stages >= n_block_min:
                self._load_kv_tile(
                    mK,
                    sK,
                    tKsK,
    ...
    def _paged_load_kv_tile(
        ...
        for m in cutlass.range_constexpr(cute.size(tKVsKV, mode=[1])):
            ...
            gKV_row = cute.tiled_divide(mKV[phys, off, kv_head, None], (async_elems,))
            for k in cutlass.range_constexpr(cute.size(tKVsKV, mode=[2])):
                ki = tKVcKV[0, 0, k][1] // async_elems
                cute.copy(gmem_tiled_copy, gKV_row[None, ki], tKVsKV[None, m, k])
    ...
    def _load_bias_tile_predicated(
        ...
        for r in cutlass.range_constexpr(cute.size(rBiasPred_mn.shape[0])):
            ...
            for c in cutlass.range_constexpr(cute.size(rBiasPred_mn.shape[1])):
                rBiasPred_mn[r, c] = row_ok and cute.elem_less(tScS_mn[r, c][3], mBias.shape[3])
        rBias.fill(0)
    ...
    def _add_bias_tile(
        ...
        * **boundary tile** (``T_q`` / ``T_k`` don't divide the tile, so the
        ...
        if cutlass.const_expr(self._bias_aligned):
            ...
            rows_interior = cute.elem_less((m_block + 1) * self._m_block_size - 1, mBias.shape[2])
            cols_interior = cute.elem_less((n_block + 1) * self._n_block_size - 1, mBias.shape[3])
        ...
        for r in cutlass.range_constexpr(cute.size(acc_S_mn.shape[0])):
            for c in cutlass.range_constexpr(cute.size(acc_S_mn.shape[1])):
                acc_S_mn[r, c] = acc_S_mn[r, c] + cutlass.Float32(rBias_mn[r, c]) * inv_scale
