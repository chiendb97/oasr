...
from benchmarks.core.metrics import percentile as _percentile
...
class RunStats:
    ...
    def pretty(self) -> str:
        ...
        def p(q):
            return _percentile(lat, q * 100.0)
...
def _build_parser() -> argparse.ArgumentParser:
    """Standalone parser, for the module's own ``main``."""
...
def parse_args(p: argparse.ArgumentParser) -> None:
    """Add the serving benchmark's flags to *p*."""
...
#: Served precision, matching what bench_service.py defaulted to.
DEFAULT_DTYPE = "bfloat16"


def _to_workload_row(
    stats: "RunStats", args: argparse.Namespace, sub: str, repro_command: str
) -> WorkloadRow:
    """Map one subroutine's RunStats onto the shared workload schema.

    Percentiles go in the row; the raw latency arrays and the rejection-cause
    histogram go to the ``.raw.json`` sidecar.  The previous JSON did the
    opposite -- it shipped every latency and no percentile, so each consumer
    recomputed them, and dropped ``error_codes`` entirely.
    """
    lat = sorted(stats.latencies_ms)
    fp = sorted(stats.first_partial_ms)
    transport = "grpc" if sub.startswith("grpc") else ("ws" if "streaming" in sub else "http")
    return WorkloadRow(
        harness="service",
        subroutine=sub,
        shape=f"c={args.concurrency},n={args.num_utterances}",
        params=params_of(
            {
        ...
        transport=transport,
        wire_encoding=args.wire_encoding,
        realtime=int(bool(args.realtime)),
        requests_ok=stats.n_ok,
        requests_rejected=stats.n_rejected,
        requests_failed=stats.n_fail,
        utterances=stats.n_ok,
        audio_s=stats.total_audio_s,
        wall_s=stats.wall_s,
        throughput_req_per_s=(stats.n_ok / stats.wall_s) if stats.wall_s > 0 else None,
        latency_mean_ms=(sum(lat) / len(lat)) if lat else None,
        latency_p50_ms=_percentile(lat, 50) if lat else None,
        latency_p90_ms=_percentile(lat, 90) if lat else None,
        latency_p95_ms=_percentile(lat, 95) if lat else None,
        latency_p99_ms=_percentile(lat, 99) if lat else None,
        latency_max_ms=max(lat) if lat else None,
        first_partial_p50_ms=_percentile(fp, 50) if fp else None,
        first_partial_p95_ms=_percentile(fp, 95) if fp else None,
        partials_per_req=(
            sum(stats.partial_counts) / len(stats.partial_counts) if stats.partial_counts else None
        ),
...
def run(args: argparse.Namespace, reporter: Reporter, repro_command: str = "") -> None:
    ...
    if getattr(args, "subroutine", None):
        args.subroutines = [args.subroutine]
    stats = _drive(args)
    for sub, s_ in stats:
        reporter.workload_row(_to_workload_row(s_, args, sub, repro_command))
        reporter.raw(
            sub,
            {
                "latencies_ms": s_.latencies_ms,
                "first_partial_ms": s_.first_partial_ms,
                "partial_counts": s_.partial_counts,
                "error_codes": s_.error_codes,
            },
        )


def main() -> int:
    """Standalone entry point; benchmarks/run.py calls :func:`run` instead."""
    args = _build_parser().parse_args()
    reporter = Reporter(output=getattr(args, "output_path", None), category="workload")
    try:
        run(args, reporter)
    finally:
        reporter.finalize()
...
def _drive(args: argparse.Namespace):
    """Spawn the server if needed, drive each subroutine, return (name, stats)."""
