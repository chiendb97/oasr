...
from benchmarks.core.driver import params_of, parse_dtype
from benchmarks.core.metrics import percentile as _shared_percentile
...
#: Served precision -- what bench_engine.py defaulted to.
...
def _percentile(sorted_values: List[float], pct: float) -> float:
    """Nearest-rank percentile -- the shared rule, so engine tick latencies
    and service request latencies are computed the same way."""
    return _shared_percentile(sorted_values, pct)
    ...
    *,
    ...
    reporter.workload_row(
        WorkloadRow(
            ...
            params=params_of(
                {
                    "utterances": n,
                    "max_batch_size": max_batch_size,
                    "chunk_size": chunk_size if is_streaming else "",
                    "admit_mode": admit_mode if family is not None else "",
                }
            ),
            architecture=getattr(engine, "architecture", "") or "",
            ckpt=ckpt_dir,
            decode_method=str(getattr(engine, "decode_method", "") or ""),
            dtype=dtype_str,
            max_batch_size=max_batch_size,
            chunk_size=chunk_size if is_streaming else None,
            service_mode="streaming" if is_streaming else "offline",
            cuda_graphs="" if use_cuda_graphs is None else ("on" if use_cuda_graphs else "off"),
            admit_mode=admit_mode if family is not None else "",
            utterances=n,
            audio_s=audio_s,
            wall_s=wall_s,
            ...
            std_ms=std_ms,
            iters=num_iters,
            # audio / wall, higher is faster.  The previous harness wrote this
            # column as wall / audio under the name `rtf`, while the service
            # harness wrote its reciprocal under the same name.
            rtfx=audio_s / wall_s if wall_s > 0 else None,
            throughput_utts_per_s=n / wall_s if wall_s > 0 else None,
            tokens=extra_metrics.get("tokens"),
            tokens_per_s=extra_metrics.get("tokens_per_sec"),
            ticks=extra_metrics.get("ticks"),
            tick_p50_ms=extra_metrics.get("tick_p50_ms"),
            tick_p99_ms=extra_metrics.get("tick_p99_ms"),
...
def _envint(name: str, default):
    """Read an int from ``$name`` if set, else *default*.

    Lets ``.env`` (``MAX_BATCH_SIZE``, ``NUM_UTTERANCES``, ...) set the default
    for an argparse int flag; the CLI flag still wins, because argparse only
    consults the default when the flag is absent.  Sizing that comes from the
    environment is why AGENTS.md rule 8 asks for ``set -a; source .env; set +a``
    before a run -- a missing one measures a different working point without
    saying so.
    """
    ...
    try:
        ...
        raise SystemExit(f"env var {name}={raw!r} is not an int") from None
...
def parse_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        help="Number of .wav files to include per benchmark run (default: $NUM_UTTERANCES or 10)",
    )
    parser.add_argument(
        "--max-batch-size",
        type=int,
    ...
    parser.add_argument(
        "--chunk-size",
        type=int,
        default=_envint("CHUNK_SIZE", None),
    ...
    parser.add_argument(
        ...
        choices=("on", "off"),
        default=None,
        help="Force CUDA-graph capture on or off (default: the engine's own choice)",
    )
    parser.add_argument(
        "--decode-admit-window-ms",
        type=float,
        default=0.0,
        help="Hold newly admitted requests this long so they batch together",
    )
    parser.add_argument(
        "--no-pinned-audio",
        action="store_true",
        help="Hand the engine pageable waveforms -- the A/B arm, not the served path",
    )


def run(args: argparse.Namespace, reporter: Reporter, repro_command: str = "") -> None:
    """Entry point called by benchmarks/run.py."""
    ...
    dtype_str = args.dtype if getattr(args, "dtype_explicit", False) else DEFAULT_DTYPE
    ...
    for cfg in _resolve_configs(args, subroutine):
        _run_config(
            ...
            decode_admit_window_ms=getattr(args, "decode_admit_window_ms", 0.0),
        )
