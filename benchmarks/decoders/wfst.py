...
def run_time(engine, gpu_batches, args):
    """Time one engine over the selected batches.

    Returns a dict of the measurement.  Timing spans read-back, host
    backtracking and word mapping, so this is decode-to-words, not the kernel.
    """
    ...
    times_ms = sorted(t * 1e3 for t, _ in per)
    audio_s = sum(a for _, a in per)
    wall_s = sum(t for t, _ in per)
    return {
...
def parse_args(parser: argparse.ArgumentParser) -> None:
    """Add the WFST decoder's flags to *parser*."""
    parser.add_argument(
        "--graph",
    ...
    parser.add_argument(
        ...
        help=f"log-prob dump (default: ${WFST_HOME_ENV}/data/logprobs/...)",
    )
    parser.add_argument("--batch", type=int, nargs="+", default=[1, 8, 32])
    parser.add_argument("--search-beam", type=float, default=20.0)
    parser.add_argument("--output-beam", type=float, default=8.0)
    parser.add_argument("--min-active", type=int, default=30)
    parser.add_argument("--max-active", type=int, default=10000)
    parser.add_argument("--repeats", type=int, default=10)
    parser.add_argument("--num-batches", type=int, default=6)
    parser.add_argument(
        "--frame-sec", type=float, default=0.04, help="seconds per frame (RTFx scale)"
    )
    ...
    parser.add_argument(
        ...
        help=f"external wfst checkout (default: ${WFST_HOME_ENV})",
    )


def run(args: argparse.Namespace, reporter: Reporter, repro_command: str = "") -> None:
    """Entry point called by benchmarks/run.py."""
    missing = [f for f, v in (("--graph", args.graph), ("--logprobs", args.logprobs)) if not v]
    if args.compare_external and not args.wfst_home:
        missing.append("--wfst-home")
    if missing:
        # The assets live outside this repo, so their absence is a skip that
        # names what to pass -- not a crash.
        reporter.warn(
            f"wfst_decoder: missing {', '.join(missing)}; pass them or set "
            f"${WFST_HOME_ENV} to the external wfst checkout"
        )
        return
    ...
    ref = args.ref_backend or "in-tree"
    for b in args.batch:
        gpu_batches = make_gpu_batches(log_probs, b, device, args.frame_sec)
        measured = {e.name: run_time(e, gpu_batches, args) for e in engines}
        ref_ms = measured.get(ref, {}).get("median_ms")
        for engine in engines:
            m = measured[engine.name]
            reporter.workload_row(
                WorkloadRow(
                    ...
                    speedup_vs_ref=(ref_ms / m["median_ms"] if ref_ms else None),
                    shape=f"B={b} beam={args.search_beam}/{args.output_beam} "
                    ...
                    latency_p99_ms=m["p99_ms"],
                    latency_max_ms=m["max_ms"],
    ...
    for engine in engines:
        if engine.overflows:
            reporter.warn(f"{engine.name}: {engine.overflows} lanes hit overflow (rescued exactly)")
