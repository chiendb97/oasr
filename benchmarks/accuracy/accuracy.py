...
from benchmarks.core.driver import params_of  # noqa: E402
from benchmarks.core.metrics import percentile as _percentile  # noqa: E402
from benchmarks.core.report import Reporter  # noqa: E402
from benchmarks.core.schema import WorkloadRow  # noqa: E402
...
def _pct(ordered: Sequence[float], q: float) -> float:
    """Percentile through the shared rule, so a p99 here and a p99 in the
    engine or service harness mean the same thing."""
    return _percentile(ordered, q * 100.0)
...
def parse_args(parser: argparse.ArgumentParser) -> None:
    """Add the accuracy sweep's flags to *parser*."""
    _add_arguments(parser)


def _to_workload_row(r: "Row", repro_command: str) -> WorkloadRow:
    """Map one sweep cell onto the shared workload schema.

    Every field name the previous 27-column CSV used is preserved, so
    ``ci/wer-reference.json`` and this row stay one vocabulary.
    """
    return WorkloadRow(
        harness="accuracy",
        subroutine="wer",
        shape=f"{r.decode_method}/{r.service_mode}/{r.dtype}/bs{r.max_batch_size}",
        params=params_of(
            {
                "decode_method": r.decode_method,
        ...
        manifest=r.manifest,
        decode_method=r.decode_method,
        decode_options=r.decode_options,
        dtype=r.dtype,
        max_batch_size=r.max_batch_size,
        chunk_size=r.chunk_size,
        service_mode=r.service_mode,
        vad_mode=r.vad_mode,
        vad_backend=r.vad_backend,
        vad_options=r.vad_options,
        utterances=r.utterances,
        audio_s=r.audio_seconds,
        wall_s=r.wall_seconds,
        rtfx=r.rtfx,
        throughput_utts_per_s=r.utt_per_second,
...
def run(args: argparse.Namespace, reporter: Reporter, repro_command: str = "") -> None:
    ...
    if getattr(args, "build_manifest", None):
        _maybe_build_manifest(args)
        return
    rows = _sweep(args)
    for row in rows:
        reporter.workload_row(_to_workload_row(row, repro_command))
    if rows:
        reporter.raw(
            "worst_utterances",
            {"config": rows[-1].manifest, "lines": rows[-1].result.worst(5)},
        )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.build_manifest:
        return _maybe_build_manifest(args)
    reporter = Reporter(
...
def _maybe_build_manifest(args: argparse.Namespace) -> int:
    if args.build_manifest:
...
def _sweep(args: argparse.Namespace):
    """Run the Cartesian sweep and return the collected :class:`Row` list."""
...
def _finish(rows, failures, args):
    if failures:
